...
local localgettimefn = function()
    ...
    local function DoBossWarning(level, times)
        ...
        local data_time = ThePlayer and ThePlayer.HUD and ThePlayer.HUD.WarningEventTimeData["gmoosespawner_time"]
        if data_time and (time < data_time) or (data_time == 0) then
            SaveTimeData("gmoosespawner", time)
        end
    end

    for i = 2, 4 do
        local level = tostring(i)
        AddPrefabPostInit("moosewarning_lvl" .. level, function(inst)
...
local remotegettextfn = function(Thread)
    if ThePlayer.HUD.WarningEventTimeData.gmoosespawner_time == 0 then return end
    ...
    BBGOAT_util:remote(cmd, nil, function(res)
        if res and res.err then
            target_name = nil
            SaveTextData("gmoosespawner", "")
            print('[警告] gmoosespawner remotegettextfn error:', res.err)
        ...
        elseif res then
            ...
            if target_name then
                local str = string.format(STRINGS.eventtimer.gmoosespawner.targeted, target_name, TimeToString(ThePlayer.HUD.WarningEventTimeData.gmoosespawner_time))
                SaveTextData("gmoosespawner", str)
            else
                local str = string.format(ReplacePrefabName(STRINGS.eventtimer.gmoosespawner.cooldown), TimeToString(ThePlayer.HUD.WarningEventTimeData.gmoosespawner_time))
                SaveTextData("gmoosespawner", str)
            end
        end
    end)
...
info = {
    ...
    remotegettimefn = function(Thread)
        GetWorldSettingsTimeLeft("mothergoose_timetoattack", nil, function(res)
            if res and res.err then
                target_name = nil
                SaveTimeData("gmoosespawner", 0)
                SaveTextData("gmoosespawner", "")
                print('[警告] gmoosespawner remotegettimefn error:', res.err)
                -- 同时删除Text线程
                local _, TextThreadList = GetRemoteThreadList()
                if TextThreadList and TextThreadList.gmoosespawner then
                    KillThreadsWithID(TextThreadList.gmoosespawner.id)
                end
                if Thread then KillThreadsWithID(Thread.id) end
            elseif res and res.time then
                SaveTimeData("gmoosespawner", res.time)
            elseif res and res.not_found then
                -- 取消数据更新任务
                if Thread then KillThreadsWithID(Thread.id) end
            end
